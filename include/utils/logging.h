...
private:
  struct Entry {
    ...
    Error Finalize() {
      return utils::Error(log_stream.str());
    }
...
#if defined(__MINGW32__) || defined(__MINGW64__)
  UTILS_NO_INLINE Entry& GetEntry() {
    return entry_;
  }
...
// #define LOG \
//   std::cerr << "[LOG]: File: " << __FILE__ << ", Line: " \
//             << __LINE__ << ": "
#define LOG(level) \
  std::cerr << "[LOG " #level << "]: File: " << __FILE__ << ", Line: " \
            << __LINE__ << ": "

#define LOG_INFO LOG(INFO)
#define LOG_ERROR LOG(ERROR)
#define LOG_WARNING LOG(WARN)
#define LOG_FATAL utils::LogMessageFatal(__FILE__, __LINE__).stream() << "[LOG FATAL]: " // NOLINT(*)
